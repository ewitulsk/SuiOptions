...
use orderbook_book::price_and_size;
use orderbook_core::order::SignedOrder;
use orderbook_core::{canonicalize_move_type, Digest, Market, Side};
use serde::Serialize;
...
pub fn validate_stateless(
    ...
    let (side, price_ticks, _base) = price_and_size(market, o).map_err(|e| match e {
        orderbook_book::BookError::WrongMarket => {
            reject(IntakeErrorCode::TokenMismatch, "tokens do not match market")
        }
        orderbook_book::BookError::OffTick => reject(IntakeErrorCode::OffTick, "price off tick"),
        orderbook_book::BookError::BelowMinSize => {
    ...
    // approved-signer set.
    let digest = orderbook_signing::order_digest(o, &signed.registry_id);
    orderbook_signing::verify_signature(
        signed.scheme,
        &digest.0,
        &signed.signature,
        &signed.public_key,
...
pub async fn intake_order(
    ...
    let derived = orderbook_signing::derive_address(signed.scheme, &signed.public_key);
    if derived != o.maker {
        let ok = state
            .store
...
mod tests {
    use super::*;
    use orderbook_core::order::{Order, SignatureScheme};
    ...
    fn signed_order(kp: &Ed25519Keypair, m: &Market, now: u64) -> SignedOrder {
        ...
        let digest = orderbook_signing::order_digest(&order, &m.registry_id);
    ...
    fn stateless_accepts_valid_order() {
        ...
        assert_eq!(digest, orderbook_signing::order_digest(&s.order, &m.registry_id));
    }
