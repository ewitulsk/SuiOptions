// Dual-axis analytics chart (SO-390): spot price (left axis, quote units)
// and annualized realized vol (right axis, percent) on one time scale.
//
...
type Hover = { time: number | null; spot: number | null; rv: number | null };
...
export function AnalyticsChart({ spot, rv, quoteSymbol }: Props) {
  const mode = useThemeMode();
  const holderRef = useRef<HTMLDivElement | null>(null);
  const chartRef = useRef<IChartApi | null>(null);
  const spotRef = useRef<ISeriesApi<"Line"> | null>(null);
  const rvRef = useRef<ISeriesApi<"Line"> | null>(null);
  const [hover, setHover] = useState<Hover>({ time: null, spot: null, rv: null });
  ...
  useEffect(() => {
    ...
    const onCrosshair = (param: MouseEventParams) => {
      if (param.time === undefined) {
        setHover({ time: null, spot: null, rv: null });
        return;
      }
