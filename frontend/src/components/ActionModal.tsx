...
export function ActionModal({ modal, spots, onSubmit, onClose }: Props) {
  ...
  if (modal.kind === "close_early") {
    const { position: p } = modal;
    const buyback = p.premiumReceived * 0.6;
    return (
      <div className="scrim">
        <div className="modal modal--wide">
          {isReview && (
            <>
              <div className="modal__head-row">
                <div className="modal__kind">close early · writer</div>
                <button className="modal__x" onClick={onClose}>×</button>
              </div>
              <div className="modal__title">Sell Position NFT to MM</div>
              <div className="modal__sub">
                best buyback quote · tidal_quants · ttl 24s
              </div>
              <div className="modal__list">
                <div className="modal__list-row">
                  <span>collateral returned</span>
                  <b className="modal__list-val--pos">
                    +{p.amount.toFixed(p.asset === "BTC" ? 4 : 0)} {p.asset}
                  </b>
                </div>
                <div className="modal__list-row">
                  <span>MM pays you</span>
                  <b className="modal__list-val--neg">−{buyback.toFixed(2)} USDC</b>
                </div>
                <div className="modal__list-row modal__list-row--quiet">
                  <span>premium kept from open</span>
                  <span>+{p.premiumReceived.toFixed(2)} USDC</span>
                </div>
                <div className="modal__list-row modal__list-row--total">
                  <span>net on this position</span>
                  <b className="modal__list-val--pos">
                    +{(p.premiumReceived - buyback).toFixed(2)} USDC
                  </b>
                </div>
              </div>
              <div className="modal__note">
                The MM takes over your range. You give up future exercise risk and get your
                collateral back today. Use this if you no longer want exposure.
              </div>
              <div className="modal__actions">
                <button className="modal__cta modal__cta--ghost" onClick={onClose}>
                  Cancel
          ...
          {(isSigning || isBroadcast) && (
            <>
              <div className="modal__spinner"></div>
              <div className="modal__title">
                {isSigning ? "Signing transfer…" : "Confirming on Sui…"}
              </div>
              <div className="modal__sub">
                {isSigning ? "waiting for wallet" : "transferring NFT to MM"}
              </div>
            </>
          )}
          {isConfirmed && (
            <>
              <div className="modal__check">✓</div>
              <div className="modal__title">Closed.</div>
              <div className="modal__sub">collateral returned to your account</div>
              <button className="modal__cta" onClick={onClose}>
                Done
              </button>
            </>
