// Taker fill PTB for the in-house exchange (SO-416). Assembles the swap from
// a `/v1/routes` quote: a chain of `settlement::fill_limit_order(_reverse)`
// calls per path/hop/leg (or `exchange_adapter::fill_vault_order(_reverse)`
// for direct-escrow vault makers, SO-372), ONE strict
// `settlement::assert_coin_min` slippage guard on the joined output, and the
// residual change back to the sender. Walks `plan.paths` (the flat
// ptbSkeleton lacks the path boundaries needed to thread hop outputs).
// Ported from exchange-dashboard/src/tx/routeFill.ts.
...
import { toBigint } from "../api/orderbook";
import { orderBytes, prefixedSignature, publicKeyBytes, type OrderFields } from "./exchangeOrders";

const VecU8 = bcs.vector(bcs.u8());

/** Wire order → the BCS-typed fields the digest/fill payload needs. */
export function orderFieldsFromWire(o: SignedOrderWire): OrderFields {
  return {
...
export function buildRouteFillTx(
  quote: RouteResponse,
  markets: Map<string, ExchangeMarket>,
  opts: {
    sender: string;
    fromType: string;
    toType: string;
    /** User's slippage floor in `toType` atomic units — replaces the
     * server's zero-tolerance min. */
    minOut: bigint;
    packageId: string;
  ...
  for (const path of quote.plan.paths) {
    ...
    path.hops.forEach((hop, hopIdx) => {
      ...
      for (const leg of hop) {
        ...
        // SO-372: a direct-escrow maker's identity BM holds nothing — its
        // legs settle from the vault's free balances through the
        ...
        const signedPayload = [
          tx.pure(VecU8.serialize(orderBytes(orderFieldsFromWire(order)))),
          tx.pure(VecU8.serialize(prefixedSignature(order.scheme, order.signature))),
          tx.pure(VecU8.serialize(publicKeyBytes(order.publicKey))),
        ];
        let res;
        if (vaultLeg) {
