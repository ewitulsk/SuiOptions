...
//! risk. The kill switch ([`KillSwitchState`]) is a pure rolling
//! high-water check over NAV samples; the desk persists its history so a
...
/// Rolling-high-water kill switch state: the NAV samples inside the
/// window, oldest first. Pure — the desk loads/persists it around
/// [`KillSwitchState::check`] (`services/mm-bot` `limits::KillSwitch`).
/// The serde shape IS the on-disk `desk-nav-history.json` format.
...
impl KillSwitchState {
    /// Record the NAV sample, drop samples older than the window, and
    /// return whether the switch is tripped: NAV more than
    /// `kill_drawdown` below the window's high water.
    pub fn check(&mut self, cfg: &LimitsConfig, nav: u64, now_ms: u64) -> bool {
        let window_ms = (cfg.kill_window_days * 86_400_000.0) as u64;
        self.samples.push((now_ms, nav));
        self.samples
            .retain(|(t, _)| now_ms.saturating_sub(*t) <= window_ms);
        let high_water = self.samples.iter().map(|(_, n)| *n).max().unwrap_or(nav);
...
mod tests {
    ...
    #[test]
    fn kill_switch_trips_on_seven_day_drawdown_and_round_trips() {
        let day = 86_400_000u64;
        let c = cfg();
        let mut k = KillSwitchState::default();
        assert!(!k.check(&c, 1_000, day));
        assert!(!k.check(&c, 950, 2 * day)); // −5%: fine
        assert!(k.check(&c, 890, 3 * day)); // −11% from high water: trip
        // The serde shape is the persisted file: a reload keeps the high
        // water.
        let json = serde_json::to_string(&k).unwrap();
        let mut k: KillSwitchState = serde_json::from_str(&json).unwrap();
