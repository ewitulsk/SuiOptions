//! Fixed-time-window price buffer the desk keeps a current sigma estimate
//! from — the ONE shared realized-vol implementation the live desk and the
//! backtester both drive (doc 08 §5.2, SO-450; relocated from
//! `pyth-client` so the strategy kernel has no network-crate dependency).
//!
//! The sampler feeding it can skip ticks while the price stream is stale,
//! so the buffer annualizes from the actual timestamps it retains rather
//! than a fixed cadence: gaps in the cadence don't bias sigma.

use std::collections::VecDeque;

use crate::rv::log_returns;
