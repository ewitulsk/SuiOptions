...
struct Cli {
    ...
    /// Path to the secrets TOML. Holds the Sui signing key (`sui.testnet`)
...
async fn main() -> Result<()> {
    ...
    let secrets_loaded = secrets::Secrets::load(&cli.secrets)
...
async fn resolve_account(
    cli: &Cli,
    cfg: &BotConfig,
    net: &clients::deployments::NetworkDeployment,
    secrets: &secrets::Secrets,
    signer: &QuoteSigner,
    pubkey_bytes: &[u8],
) -> Result<ObjectID> {
