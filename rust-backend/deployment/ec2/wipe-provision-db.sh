...
#
#   <env>     staging | prod   (mainnet/production BLOCKED)
#   <service> indexer | scheduler | token-info | orderbook
#
...
# standalone wipe workflow only offers staging; redeploy-contract
...
case "$ENV" in
  ...
  staging|prod) : ;;
  *)
    echo "FATAL: unknown env '$ENV' (expected staging|prod)"
    exit 1
    ;;
esac
