...
//!   require an admin JWT; solana-token-info doesn't validate it itself — the
//!   `auth-client` middleware delegates to solana-auth-service's internal
//!   `/verify`.
...
/// Public API: open reads + JWT-gated writes. `auth` points at
/// solana-auth-service's internal verify route.
pub fn public_router(
    ...
    let reads = Router::new()
        ...
        .route("/program-info", get(tokens::program_info));
...
pub fn internal_router(state: Arc<AppState>) -> Router {
    Router::new()
        .route("/health", get(health))
        .route("/tokens", post(tokens::create_token))
        .route("/tokens/:mint", put(tokens::update_token))
        .route("/tokens/:mint", delete(tokens::delete_token))
        .with_state(state)
...
pub async fn serve_public(
    ...
    info!(%addr, "solana-token-info public API listening");
...
pub async fn serve_internal(addr: SocketAddr, state: Arc<AppState>) -> Result<()> {
    ...
    let listener = tokio::net::TcpListener::bind(addr).await?;
    info!(%addr, "solana-token-info internal API listening");
    axum::serve(listener, app).await?;
    Ok(())
}
