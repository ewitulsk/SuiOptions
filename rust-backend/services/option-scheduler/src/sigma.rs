//! Realized-vol source for the z-ladder grid (doc 05 §4.2): annualized σ
//! from daily closes fetched via Pyth Benchmarks, one point per day of
//! the window. Rolls are weekly, so the call volume is negligible.

use anyhow::{bail, Context, Result};
use tracing::debug;

use pyth_client::types::PriceFeedId;
use pyth_client::vol::{log_returns, realized_vol};
...
pub async fn realized_sigma_from_benchmarks(
    ...
    for i in (0..=window_days as i64).rev() {
        ...
        let update = pyth_client::benchmark_at(client, benchmarks_url, feed, t)
            .await
