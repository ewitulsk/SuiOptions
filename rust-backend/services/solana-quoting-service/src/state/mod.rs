...
//! they're read just-in-time from the solana-indexer's GraphQL API
//! ([`solana_indexer_graphql::IndexerClient`]). The only state this service
//! owns is what the indexer can't tell it: in-flight quote reservations and
//! the per-MM reputation score.
...
pub use solana_indexer_graphql::{Account, Bucket, IndexerClient};
...
/// orchestrator's matcher (or the bulk-view collector). The first element is
/// the MM's account id (base58).
#[derive(Debug)]
pub enum MmResponse {
    Quote(String, crate::messages::MmQuotePayload),
    Decline(String),
    /// Unsigned indicative premiums for a bulk-view RFQ. Routed through the
    /// same `pending_rfqs` channel keyed by request_id.
    BulkView(String, crate::messages::BulkViewQuotePayload),
}
...
pub struct AppState {
    ...
    reconcile_cursors: DashMap<String, u64>,
    ...
    pub bulk_view_cache: DashMap<(String, u64), BulkViewCacheEntry>,
    ...
    bulk_view_refreshing: DashMap<(String, u64), ()>,
...
impl AppState {
    ...
    pub fn try_claim_bulk_view_refresh(&self, key: (String, u64)) -> bool {
        self.bulk_view_refreshing.insert(key, ()).is_none()
    }

    pub fn release_bulk_view_refresh(&self, key: &(String, u64)) {
        self.bulk_view_refreshing.remove(key);
    }

    /// Spendable balance for `(account, mint)`: the JIT-fetched on-chain
    /// balance minus our own active reservations in that mint. Callers must
    ...
    pub fn available(&self, account: &Account, mint: &str) -> u64 {
        account
            .balance(mint)
            .saturating_sub(self.reservations.active_amount(&account.account_id, mint))
    }
    ...
    pub async fn reconcile_executed(&self, account: &str) -> anyhow::Result<()> {
        let after = self
            .reconcile_cursors
            .get(account)
            .map(|c| *c)
        ...
        if max_seq > after {
            self.reconcile_cursors.insert(account.to_string(), max_seq);
        }
        Ok(())
    }
}
...
mod tests {
    ...
    fn account_with(account_id: &str, usdc: u64) -> Account {
        Account {
            account_id: account_id.into(),
            owner: "owner".into(),
            signing_scheme: 0,
            signing_pubkey: vec![0xab; 32],
            balances: BTreeMap::from([(USDC.to_string(), usdc)]),
        }
    }
    ...
    fn available_subtracts_active_reservations() {
        ...
        let acct = account_with("acc111", 1_000);
        assert_eq!(s.available(&acct, USDC), 1_000);
        ...
        assert_eq!(s.available(&acct, USDC), 700);
        // A different mint is unaffected (no balance → 0).
        assert_eq!(s.available(&acct, TBTC), 0);
    }
}
