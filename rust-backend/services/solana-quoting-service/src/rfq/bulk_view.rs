...
use solana_indexer_graphql::Bucket;

use crate::messages::{BulkViewPremium, BulkViewRfqBroadcastPayload, ServiceToMm};
...
pub async fn orchestrate_bulk_view(
    ...
    let requested: Vec<String> = buckets.iter().map(|b| b.bucket_id.clone()).collect();
    ...
    for b in &buckets {
        match state
            .bulk_view_cache
            .get(&(b.bucket_id.clone(), write_amount))
        {
...
fn build_response(
    ...
    for bucket_id in requested {
        if let Some(e) = state
            .bulk_view_cache
            .get(&(bucket_id.clone(), write_amount))
        {
...
async fn refresh_buckets(
    ...
    for b in buckets {
        if state.try_claim_bulk_view_refresh((b.bucket_id.clone(), write_amount)) {
            claimed.push(b);
        }
    }
    if claimed.is_empty() {
    ...
    let _guard = ClaimGuard {
        ...
        keys: claimed
            .iter()
            .map(|b| (b.bucket_id.clone(), write_amount))
            .collect(),
    };
    ...
    // from solana-api-service itself.
    let bucket_ids: Vec<String> = claimed.iter().map(|b| b.bucket_id.clone()).collect();
    ...
    let mut by_bucket: HashMap<String, Vec<u64>> = HashMap::new();
    if expected > 0 {
        ...
        if let Some(premiums) = by_bucket.get(&b.bucket_id) {
            if !premiums.is_empty() {
                state.bulk_view_cache.insert(
                    (b.bucket_id.clone(), write_amount),
                    BulkViewCacheEntry {
...
mod tests {
    ...
    fn mk_bucket(tag: u8) -> Bucket {
        Bucket {
            bucket_id: bs58::encode([tag; 32]).into_string(),
            underlying_mint: TBTC.into(),
            settlement_mint: USDC.into(),
            option_mint: bs58::encode([0x77u8; 32]).into_string(),
