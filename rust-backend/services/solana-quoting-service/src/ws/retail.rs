...
//! eventual on-chain transaction themselves.
//!
...
//!   current state of each subscribed bucket.
...
pub async fn handle(
    ...
    // global cap on AppState, this prevents one client from spawning
        ...
        match msg {
            ...
            RetailToService::RFQRequest { request_id, payload } => {
                ...
                let _ = state.rfq_observers.send(RfqObservation {
                    ...
                    bucket_id: payload.bucket_id.clone(),
                    write_amount: payload.write_amount,
                    side: payload.side,
                });
                // Resolve the bucket JIT. This lets us short-circuit
                // invalidated/unknown buckets with an explicit error instead
                // of a generic "no quotes" timeout.
