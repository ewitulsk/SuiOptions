...
//!    pubkey the indexer holds for the claimed MmAccount (ed25519 only).
...
//!
//! [`MmRegistry`]: crate::state::MmRegistry
...
pub async fn handle(
    ...
    // Fetched JIT; an indexer error or unknown account leaves the scheme
    // `None`, which `verify_challenge_response` treats as "scheme unknown"
    // (fail closed).
    ...
    let indexer_scheme = indexer_view.as_ref().map(|a| a.signing_scheme);
    ...
    if let Err(reason) = verify_challenge_response(
        ...
        let (code, message): (&str, String) = match reason {
            crate::ws::auth::AuthError::SchemeUnknown => (
                "auth_scheme_unknown",
                format!(
                    "either the solana-indexer has not yet ingested an \
                     AccountCreated event for {} (check it is running against \
                     the current deployment's program ids and has caught up \
                     to the slot that created this MmAccount), or the \
                     registered/supplied signing_scheme is not Ed25519 (0) — \
                     the only scheme program v1 supports",
                    hello.account_id
                ),
            ),
            crate::ws::auth::AuthError::PubkeyMismatch => (
                ...
                format!(
                    "Hello's (scheme, pubkey) does not match what is on chain \
                     for MmAccount {} — verify the mm-bot quote_key matches \
                     the registered signing_pubkey",
                    hello.account_id
                ),
            ),
        ...
        sink.send(Message::Text(serde_json::to_string(&ServiceToMm::Error {
            request_id: None,
            payload: crate::messages::ErrorPayload {
                code: code.into(),
                message,
            },
        })?))
            ...
            match msg {
                MmToService::Quote { request_id, payload } => {
                    ...
                    let tx = read_state
                        ...
                        .map(|e| e.value().clone());
                    if let Some(tx) = tx {
                        let _ = tx.send(MmResponse::Quote(account_id.clone(), payload)).await;
                    } else {
                ...
                MmToService::Decline { request_id, .. } => {
                    ...
                    let tx = read_state
                        ...
                        .map(|e| e.value().clone());
                    if let Some(tx) = tx {
                        let _ = tx.send(MmResponse::Decline(account_id.clone())).await;
                    }
                }
                MmToService::BulkViewQuote { request_id, payload } => {
                    ...
                    if let Some(tx) = tx {
                        let _ = tx
                            .send(MmResponse::BulkView(account_id.clone(), payload))
                            .await;
                    } else {
