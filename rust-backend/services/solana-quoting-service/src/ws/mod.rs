...
//!
//! - The MM `Hello` payload carries `account_id` and `signing_pubkey`.
//! - The retail `Hello` payload carries `role` and `version`.
//!
...
pub async fn serve(addr: SocketAddr, state: Arc<AppState>, cfg: Arc<Config>) -> Result<()> {
    ...
    info!(%addr, "solana-quoting-service ws listening");
...
async fn dispatch(
    ...
    if payload.get("account_id").is_some() {
        // MM Hello shape.
        let hello: crate::messages::MmHelloPayload = serde_json::from_value(payload.clone())?;
        debug!(?peer, account = %hello.account_id, "mm hello");
        mm::handle(ws, peer, state, cfg, hello).await
    } else {
        let hello: crate::messages::RetailHelloPayload = serde_json::from_value(payload.clone())?;
