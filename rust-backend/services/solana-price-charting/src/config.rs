...
//! `${SOLANA_CHART_DATABASE_URL}` expands from the environment at boot (the
//! URL is a secret — it never appears in the TOML itself).
//!
//! Deliberately smaller than the Sui twin's: no `network`/`rpc_url` (no
//! chain reads), no `api_service_url` (that was DeepBook discovery), and no
//! discovery/poll/mid-sample intervals (no watcher or mid sampler is ported
//! until a Solana order-book source exists). `ttl_hours` is kept — eviction
//! still guards future ingestion.
...
fn default_oracle_url() -> String {
    "http://solana-oracle-service:9013".into()
}
...
pub struct Config {
    ...
    /// Tiger Data TimescaleDB URL, injected as `${SOLANA_CHART_DATABASE_URL}`.
    pub database_url: String,
    #[serde(default = "default_db_pool_size")]
    pub db_pool_size: u32,

    /// Token catalog (decimals + Pyth feed ids for the apy sampler).
    pub token_info_url: String,
    /// solana-indexer GraphQL endpoint — the apy sampler lists vaults /
    /// rounds / auctions and pulls the realized-APY series from here.
    pub indexer_graphql_url: String,
    /// solana-oracle-service base URL — the apy sampler's source for spot +
    /// realized vol.
    #[serde(default = "default_oracle_url")]
    pub oracle_url: String,

    /// Trades for pools that left the tradeable set are dropped once their
    /// newest row is older than this. Unused until ingestion lands.
...
/// Realized-vol + spot-guard knobs for the apy sampler. Prices and vol come
/// from solana-oracle-service; these only tune the consumer-side guards and
/// window.
#[derive(Debug, Clone, Deserialize)]
...
/// cadence are NOT here — they're the vault's on-chain config, read
/// per-vault from the served config. These are genuine model knobs.
#[derive(Debug, Clone, Deserialize)]
pub struct ModelConfig {
    /// Number of future rounds to forecast (Tier 2 horizon, K).
    #[serde(default = "default_horizon")]
    pub forecast_horizon: u32,
    /// Strike the Tier-2 forecast prices, as a call delta (~0.10).
    #[serde(default = "default_delta")]
    pub delta_target: f64,
    /// Clamp annualized APY to ±this fraction (e.g. 5.0 = ±500%). Stops a
    /// short-cadence (hourly) round's tiny premium from annualizing to
    /// absurd numbers.
    ...
    /// assumed_vrp`). At 0 the premium exactly offsets expected assignment
    /// and the net APY is ≈ −fees (no free lunch).
