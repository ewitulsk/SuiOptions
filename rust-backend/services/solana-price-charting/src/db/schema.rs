...
//!
//! Same tables as the Sui twin; the trade idempotency column is `signature`
//! (Solana tx signature) instead of `tx_digest` — fresh DB, no compat burden.

diesel::table! {
    pool_trades (pool_id, signature, event_index) {
        time          -> Timestamptz,
        pool_id       -> Text,
        bucket_id     -> Text,
        price         -> Double,
