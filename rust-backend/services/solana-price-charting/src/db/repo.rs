...
//!
//! The trade/mid write paths (`insert_trades`, `insert_mid`, `evict_stale`)
//! have no caller yet — they're the surface a future Solana order-book
//! ingestion task writes through. The read paths simply serve empty results
//! until then.
...
impl Repo {
    ...
    pub fn insert_trades(&self, trades: &[TradeRow], cursor: (String, i64)) -> Result<usize> {
        ...
        metrics::histogram!("solana_price_charting_db_write_duration_seconds")
