//! DEPRECATED (SO-332) — the covered-call ("Ribbon-style") vault crank.
//!
//! The covered-call vault product is retired: `contracts/vault` is no
//! longer published (see `contracts/vault/DEPRECATED.md`), the scheduler
//! provisions no vaults, and the deployed `keeper` binary runs only the
//! trading-vault pass. This module is the old crank, moved here verbatim
//! so it keeps compiling and stays readable — nothing in `deployment/`
//! or `.github/workflows/` builds or runs it.
//!
//! It is reachable only through the `keeper-legacy` binary
//! (`src/bin/keeper-legacy.rs`), which is never containerized. Running it
//! against a deployment whose token-info carries no `vault` package fails
//! at boot, by design: there is nothing to crank.
//!
//! Boot:
...
//!      ([`crate::discovery`]).
//!   2. Per vault: read the chain ([`crate::state::fetch_vault_view`] +
//!      AuctionCreated discovery), plan the single next action
//!      ([`crate::planner::plan`]), resolve a strike pick when asked, and
//!      submit with a Pyth price update prepended in-PTB
//!      ([`crate::submit`]).
//!
...
use crate::config::{KeeperConfig, VaultDefaults};
use crate::discovery::{
    resolve_price_info_table_from, resolve_vault, DiscoveredVault, PriceInfoTable,
};
use crate::planner::{plan, Action, BucketMeta, PlanInput};
use crate::state::{discover_open_auctions, fetch_vault_view, VaultView};
use crate::strike::{pick_bucket, BucketCandidate};
use crate::submit::{classify, execute, execute_select_bucket, ErrorClass, SubmitCtx};
use crate::Cli;
...
/// Boot and run the covered-call crank loop. Never returns.
pub async fn run() -> Result<()> {
    ...
    // Cranks target options_vault; auction discovery filters the generic
    // auction package's events. Both required — unlike the deployed keeper,
    // this binary has nothing to do without them.
    let vault_package = snapshot
        .vault()
        .context(
            "options_vault package missing from token-info package_info — the covered-call \
             vault product is deprecated (SO-332) and this deployment no longer publishes it",
        )?
        .package()?;
    ...
    // The keeper's own `http` stays for the on-chain VAA path (submit.rs),
    // which a price cache can't serve. Hard cutover: crash if it never comes up.
    ...
    // snapshot, the package ids, the Sui client and the oracle handshake.
    // Nothing below this line can fail startup, so this is where "started"
    // becomes "ready" (SO-324).
    readiness.ready();
...
async fn tick_vault(
    ...
    let slices = crate::slicing::effective_slices(
        ids.defaults.slicing.slices,
        view.config.selling_window_ms,
    );
    let stagger_ms = crate::slicing::effective_stagger_ms(
