//! `GET /options/metrics` — Black-Scholes greeks, implied vol, fair value, and
//! break-even for a single call. STATELESS: every input is a query param, so
//! api-service stays a pure compute endpoint (no Pyth / DeepBook reads). The
//! frontend holds live spot (Pyth) and the order-book mid (DeepBook devInspect)
//! and passes them in.
//!
...
/// Default risk-free rate when `r` is omitted. The MM bot reads `cfg.rate`
/// (annualized continuous); 0 keeps greeks comparable to a retail screen that
/// ignores rates. Bump here if we later thread the env's configured rate in.
