//! Spot-cross resolution against oracle-service.
//!
//! The USD-cross math (per-leg confidence + publish-lag guards, then divide)
//! is unchanged from when this read Pyth directly; only the source moved to
//! oracle-service, which serves raw per-feed `price`/`conf`/`publish_time_ms`
//! so each consumer keeps its own guards.
