...
use super::models::{CursorRow, TradeRow};
...
impl Repo {
    ...
    /// Drop trades for pools outside `keep` whose newest trade is older than
    /// `ttl_hours`. Returns (pools dropped, rows deleted).
    pub fn evict_stale(&self, keep: &[String], ttl_hours: i64) -> Result<(usize, usize)> {
        let mut conn = self.conn()?;
        let stale: Vec<String> = diesel::sql_query(
            "SELECT pool_id, last(bucket_id, time) AS bucket_id, \
                    last(price, time) AS last_price, \
                    (extract(epoch FROM max(time)) * 1000)::int8 AS last_trade_ms, \
                    0::float8 AS volume_24h_base \
             FROM pool_trades GROUP BY pool_id \
             HAVING max(time) < now() - make_interval(hours => $1)",
        )
