...
//! wallet accumulates one object per funding-pass deposit (SO-375 LPs
//! with an infinite pocket, so this grows without bound if left alone).
//! Transfers emit no events, so the indexer cannot know what
