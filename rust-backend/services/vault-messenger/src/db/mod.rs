//! Postgres persistence. Same diesel + r2d2 + embedded-migration shape as
//! the cctp-relay / indexer `db` modules.
...
pub fn establish_pool(database_url: &str, max_size: u32) -> Result<DbPool> {
    ...
    Pool::builder()
        .max_size(max_size)
        .build(manager)
        .context("building r2d2 pool for the vault-messenger DB")
}
