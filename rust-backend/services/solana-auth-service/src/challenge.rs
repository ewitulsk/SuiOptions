...
impl ChallengeStore {
    ...
    ///
    /// The `(solana)` prefix is deliberate: it makes the message distinct from
    /// the Sui auth-service's challenge, so a signature for one chain's
    /// challenge can never be replayed against the other service.
    pub fn issue(&self) -> String {
        let mut nonce = [0u8; 32];
        rand::rngs::OsRng.fill_bytes(&mut nonce);
        let message = format!(
            "SuiOptions admin login (solana)\nnonce: {}",
            hex::encode(nonce)
        );
...
mod tests {
    ...
    #[test]
    fn message_has_solana_prefix() {
        let s = ChallengeStore::new(300);
        let m = s.issue();
        assert!(m.starts_with("SuiOptions admin login (solana)\nnonce: "));
    }
}
