...
use solana_api_service::{catalog::TokenCatalog, router, AppState, Cli, Config};
use solana_token_info_client::TokenInfoClient;

#[tokio::main]
async fn main() -> Result<()> {
    let _obs = observability::init("solana-api-service");
    ...
    // Fetch the supported-token catalog from solana-token-info. Hard
