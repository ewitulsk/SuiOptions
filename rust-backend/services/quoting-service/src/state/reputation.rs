...
pub struct ReputationStore {
    by_account: DashMap<ObjectId, Mutex<ReputationStats>>,
}
...
impl ReputationStore {
    ...
    fn with_mut<R>(&self, id: ObjectId, f: impl FnOnce(&mut ReputationStats) -> R) -> R {
        let entry = self
            .by_account
            .entry(id)
    ...
    pub fn record_signed(&self, id: ObjectId) {
        self.with_mut(id, |s| s.quotes_signed += 1);
    }

    pub fn record_executed(&self, id: ObjectId) {
        self.with_mut(id, |s| s.quotes_executed += 1);
    }

    pub fn record_expired_unused(&self, id: ObjectId) {
        self.with_mut(id, |s| s.quotes_expired_unused += 1);
    }

    pub fn record_reverted(&self, id: ObjectId) {
        self.with_mut(id, |s| s.quotes_reverted += 1);
    }

    pub fn snapshot(&self, id: &ObjectId) -> ReputationStats {
        self.by_account
            .get(id)
            .map(|e| *e.lock())
            .unwrap_or_default()
...
mod tests {
    ...
    fn store_records_per_account() {
        ...
        let a = ObjectId::new([0x01; 32]);
        let b = ObjectId::new([0x02; 32]);
