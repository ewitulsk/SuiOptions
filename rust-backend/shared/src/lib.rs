...
/// Generate a `pub fn program_spec() -> shared::program_spec::ProgramSpec` for
/// the calling binary crate. The clap `CommandFactory` is wrapped so the TUI
...
macro_rules! define_program {
    (
        ...
        pub fn program_spec() -> $crate::program_spec::ProgramSpec {
            $crate::program_spec::ProgramSpec::new(
                $id,
                $pkg,
                $dir,
                $desc,
