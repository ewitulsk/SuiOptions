...
async fn main() -> anyhow::Result<()> {
    ...
    let store = data_room_store::open(&cfg.store_url)?;
...
fn route_payload(exchange: &str, payload: &str) -> Option<String> {
    match exchange {
        "hyperliquid" => data_room_adapters::hyperliquid::route(payload),
        "bluefin" => data_room_adapters::bluefin::route(payload),
        _ => data_room_adapters::coinbase::route(payload),
    }
}
