...
async fn normalize_stream_day(store: &Store, stream: &str, date: &str) -> anyhow::Result<()> {
    let prefix = data_room_schema::bronze_ws_prefix("deribit", stream, date);
    ...
    let mut by_underlying: std::collections::BTreeMap<String, Vec<data_room_schema::OptionsQuote>> =
        Default::default();
    ...
    for key in &keys {
        ...
        for (i, line) in raw.lines().enumerate() {
            ...
            let Ok(bl) = serde_json::from_str::<BronzeLine>(line) else {
                rejects.push(data_room_adapters::Reject {
                    src_file: key.clone(),
                    src_line: i as i32,
            ...
            match data_room_adapters::deribit::parse_book_summary(
                &payload,
                Some(bl.ts_recv_ns),
                key,
                i as i32,
                ...
                Ok(quotes) => {
                    for q in quotes {
                        let u = data_room_adapters::deribit::underlying_of(&q.instrument_id)
    ...
    for (underlying, rows) in by_underlying {
        rows_total += rows.len();
        let bytes =
            data_room_schema::write_parquet(&data_room_schema::options_quotes_batch(rows)?)?;
        put_bytes(
            store,
            &data_room_schema::options_silver_key("deribit", &underlying, date),
            bytes,
        )
        .await?;
    }
    ...
    for currency in currencies {
        ...
        for (day, rows) in by_day {
            let batch = RecordBatch::try_new(
                Arc::new(data_room_schema::vol_index_schema()),
                vec![
            ...
            put_bytes(
                store,
                &data_room_schema::vol_index_key("deribit", &symbol, &day),
                bytes,
            )
            .await?;
            partitions += 1;
...
pub async fn fetch_instruments(
    currency: &str,
) -> anyhow::Result<Vec<data_room_schema::Instrument>> {
    ...
    Ok(resp
        ...
        .filter_map(|i| {
            let (underlying, expiry, strike, opt_type) =
                data_room_adapters::deribit::parse_instrument_name(&i.instrument_name)?;
            Some(data_room_schema::Instrument {
                instrument_id: data_room_adapters::deribit::instrument_id(&i.instrument_name),
