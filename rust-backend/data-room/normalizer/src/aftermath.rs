...
pub async fn normalize_day(store: &Store, date: &str) -> anyhow::Result<usize> {
    ...
    let mut by_pair: BTreeMap<String, Vec<data_room_schema::QuoteLadder>> = Default::default();
    let mut rejects = Vec::new();
    let mut lines_total = 0usize;
    for stream in &streams {
        let prefix = data_room_schema::bronze_ws_prefix("aftermath", stream, date);
        for key in list_keys(store, &prefix).await? {
            lines_total +=
                for_each_bronze_payload(store, &key, &mut rejects, |line_no, ts, payload| {
                    let q = data_room_adapters::aftermath::parse(payload, ts, &key, line_no)?;
    ...
    for (pair, rows) in by_pair {
        rows_total += rows.len();
        let bytes = data_room_schema::write_parquet(&data_room_schema::quote_ladder_batch(rows)?)?;
        put_bytes(
            store,
            &data_room_schema::quote_ladder_key("aftermath", &pair, date),
            bytes,
        )
        .await?;
    }
