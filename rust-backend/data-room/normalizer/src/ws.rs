...
use data_room_schema::CanonicalEvent;
...
fn parse_payload(
    ...
    src_file: &str,
    src_line: i32,
) -> Result<Vec<CanonicalEvent>, data_room_adapters::Reject> {
    match exchange {
        "hyperliquid" => {
            data_room_adapters::hyperliquid::parse(payload, ts_recv, src_file, src_line)
        }
        _ => data_room_adapters::coinbase::parse(payload, ts_recv, src_file, src_line),
    }
}
...
fn stream_target(exchange: &str, stream: &str) -> Option<(&'static str, String)> {
    ...
    match (exchange, kind) {
        ...
        ("hyperliquid", "trades") => Some((
            "trades",
            data_room_adapters::hyperliquid::partition_symbol(suffix),
        )),
        ("hyperliquid", "bbo") => Some((
            "book_top",
            data_room_adapters::hyperliquid::partition_symbol(suffix),
        )),
...
pub async fn normalize_stream_day(
    ...
    let prefix = data_room_schema::bronze_ws_prefix(exchange, stream, date);
    ...
    let written = match table {
        "trades" => {
            ...
            if n > 0 {
                let bytes =
                    data_room_schema::write_parquet(&data_room_schema::trades_batch(rows)?)?;
                put_bytes(
                    store,
                    &data_room_schema::silver_key("trades", exchange, &symbol, date),
                    bytes,
                )
                .await?;
            }
        ...
        "book_top" => {
            ...
            if n > 0 {
                let bytes =
                    data_room_schema::write_parquet(&data_room_schema::book_top_batch(rows)?)?;
                put_bytes(
                    store,
                    &data_room_schema::silver_key("book_top", exchange, &symbol, date),
                    bytes,
                )
                .await?;
            }
        ...
        "funding_rates" => {
            ...
            if n > 0 {
                ...
                put_bytes(
                    store,
                    &data_room_schema::funding_silver_key(exchange, &symbol, date, "predicted"),
                    bytes,
                )
                .await?;
            }
