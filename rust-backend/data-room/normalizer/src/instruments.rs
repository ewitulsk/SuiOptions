...
use data_room_schema::instrument::{instruments_batch, instruments_key, Instrument};
...
pub fn binance_static(native: &str) -> anyhow::Result<Instrument> {
    let (base, quote) = data_room_adapters::binance_vision::split_symbol(native)
...
pub fn binance_perp_static(native: &str) -> anyhow::Result<Instrument> {
    let (base, quote) = data_room_adapters::binance_vision::split_symbol(native)
        .ok_or_else(|| anyhow::anyhow!("bad binance symbol {native}"))?;
    Ok(Instrument {
        instrument_id: data_room_adapters::binance_vision::perp_instrument_id(native)
...
pub fn hyperliquid_static(coin: &str) -> Instrument {
    Instrument {
        instrument_id: data_room_adapters::hyperliquid::instrument_id(coin),
...
pub async fn snapshot(
    ...
    let n = rows.len();
    let bytes = data_room_schema::write_parquet(&instruments_batch(rows)?)?;
