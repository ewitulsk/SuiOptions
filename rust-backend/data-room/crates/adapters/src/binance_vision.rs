...
pub fn parse_book_ticker_csv<R: Read>(
    ...
    native_symbol: &str,
    src_file: &str,
) -> Result<(Vec<data_room_schema::BookTop>, Vec<Reject>), anyhow::Error> {
...
pub fn for_each_book_ticker<R: Read>(
    ...
    instrument: &str,
    src_file: &str,
    mut on_row: impl FnMut(data_room_schema::BookTop),
    mut on_reject: impl FnMut(Reject),
) -> Result<(), anyhow::Error> {
    let instrument = instrument.to_string();
    let mut rdr = csv::ReaderBuilder::new()
    ...
    for (i, rec) in rdr.records().enumerate() {
        ...
        let parsed = (|| -> Option<data_room_schema::BookTop> {
...
pub fn parse_funding_csv<R: Read>(
    ...
    native_symbol: &str,
    src_file: &str,
) -> Result<(Vec<data_room_schema::FundingRate>, Vec<Reject>), anyhow::Error> {
    let instrument = perp_instrument_id(native_symbol)
        .ok_or_else(|| anyhow::anyhow!("not a perp symbol: {native_symbol}"))?;
    let mut rdr = csv::ReaderBuilder::new()
        .has_headers(false)
    ...
    for (i, rec) in rdr.records().enumerate() {
        ...
        let parsed = (|| -> Option<data_room_schema::FundingRate> {
            Some(data_room_schema::FundingRate {
