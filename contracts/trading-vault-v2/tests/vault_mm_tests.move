#[test_only]
/// Direct port of the v1 vault_mm tests. Behavior delta in v2: exits and
/// `release` route through risk-off gating (release aborts 124; curator
/// sessions open forced ⇒ `take` aborts 91), so setups that draw funds
/// fund the curator commitment first — free-balance expectations shift by
/// the commitment amount.
module vault_v2::vault_mm_tests;
...
fun release_pulls_collateral_when_enabled_and_bound_to_vault() {
    ...
    h::fund_commitment(&mut sc, 100_000, &clock);
    ...
    assert!(vault::free_balance_of<h::USDC>(&v) == 850_000);
...
#[test]
#[expected_failure(abort_code = 3, location = vault_v2::vault_mm)]
...
fun release_rejected_when_risk_off() {
    // v2 delta: an enabled release path still refuses to draw funds while
    // the curator commitment is unfunded (risk-off, §8.4b).
    let mut sc = ts::begin(h::admin_addr());
    let clock = h::init_protocol(&mut sc);
    h::new_default_vault(&mut sc, &clock);
    h::simple_deposit(&mut sc, h::alice_addr(), 1_000_000, &clock);
...
#[test]
#[expected_failure(abort_code = 2, location = vault_v2::vault_mm)]
...
#[test]
#[expected_failure(abort_code = 1, location = vault_v2::vault_mm)]
...
fun held_put_coin_appraises_at_intrinsic_then_zero_after_expiry() {
    ...
    let vault_id = h::new_default_vault(&mut sc, &clock);
    h::simple_deposit(&mut sc, h::alice_addr(), 1_000_000, &clock);
...
fun exercise_call_coin_partial_then_appraisal_completes() {
    ...
    assert!(vault::free_balance_of<h::USDC>(&v) == 1_020_000);
    ...
    // 1_020_000 + 40_000×3 BTC + 60_000×(3−2) coin intrinsic.
    ...
    assert!(vault::appraisal_value(&appraisal) == 1_020_000 + 120_000 + 60_000);
    let wl = ts::take_shared<Whitelist>(&sc);
    let position = vault::deposit<h::USDC>(
        &mut v,
        &cfg,
        &wl,
        appraisal,
...
#[test]
#[expected_failure(abort_code = 78, location = vault_v2::vault)]
fun exercise_call_coin_insufficient_settlement_aborts() {
    let mut sc = ts::begin(h::admin_addr());
    let clock = h::init_protocol(&mut sc);
    let vault_id = h::new_default_vault(&mut sc, &clock);
    // Not enough USDC for the 80_000 strike payment (commitment 10_000 +
    // deposit 50_000 keeps the vault risk-on but underfunded).
    h::fund_commitment(&mut sc, 10_000, &clock);
...
fun exercise_put_coin_delivers_underlying_and_collects_payout() {
    ...
    assert!(vault::free_balance_of<h::USDC>(&v) == 1_180_000);
    assert!(vault::position_count(&v) == 1);
...
fun close_offset_put_position_returns_cash_collateral() {
    ...
    assert!(vault::free_balance_of<h::USDC>(&v) == 1_300_000);
    assert!(vault::position_count(&v) == 0);
...
fun release_coin_to_balances_makes_coin_appraisable_as_balance() {
    ...
    assert!(vault::appraisal_value(&appraisal) == 1_100_000 + 50_000);
    let wl = ts::take_shared<Whitelist>(&sc);
    let position = vault::deposit<h::USDC>(
        &mut v,
        &cfg,
...
/// any session opens. (Forced/crank sessions cannot reach the
/// balance-drawing exits at all — they `take`, and forced sessions abort
/// 91 on `take` by construction.)
#[test]
