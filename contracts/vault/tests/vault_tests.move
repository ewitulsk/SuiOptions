#[test_only]
module options_vault::vault_tests;
...
use options_vault::test_helpers::{Self as th, BTC, USDC, CALL};
use options_vault::vault::{Self, Vault, DepositReceipt, WithdrawReceipt};
...
fun bid_as(
    ...
    ts::next_tx(scenario, who);
    let mut a = ts::take_shared_by_id<Auction<BTC, USDC>>(scenario, rfq_id);
    auc::bid(
        &mut a,
...
fun settle_rfq_as(
    ...
    ts::next_tx(scenario, who);
    let mut v = take_vault(scenario);
    let a = ts::take_shared_by_id<Auction<BTC, USDC>>(scenario, rfq_id);
...
fun swap_bid_as(
    ...
    ts::next_tx(scenario, who);
    let mut a = ts::take_shared_by_id<Auction<USDC, BTC>>(scenario, swap_id);
    auc::bid(
        &mut a,
...
fun settle_swap_at(
    ...
    ts::next_tx(scenario, who);
    let mut v = take_vault(scenario);
    let a = ts::take_shared_by_id<Auction<USDC, BTC>>(scenario, swap_id);
...
fun test_full_round_lifecycle_with_partial_exercise() {
    ...
    ts::next_tx(&mut scenario, keeper());
    let a = ts::take_shared_by_id<Auction<BTC, USDC>>(&scenario, rfq_id);
    assert!(auc::reserve_bid(&a) == 28_571, 0);
    ts::return_shared(a);
...
fun test_fee_cap_clamps_to_round_profit() {
    ...
    assert!(options_core::treasury::balance_of<BTC>(&treasury) == 1_000, 0);
    ts::return_shared(treasury);
...
#[test]
#[expected_failure(abort_code = 35, location = options_vault::vault)] // vault_wrong_phase
...
#[test]
#[expected_failure(abort_code = 37, location = options_vault::vault)] // vault_bucket_already_selected
...
#[test]
#[expected_failure(abort_code = 36, location = options_vault::vault)] // vault_bucket_not_selected
...
#[test]
#[expected_failure(abort_code = 38, location = options_vault::vault)] // vault_selling_closed
...
#[test]
#[expected_failure(abort_code = 45, location = options_vault::vault)] // vault_slice_too_large
...
#[test]
#[expected_failure(abort_code = 40, location = options_vault::vault)] // vault_rfqs_open
...
#[test]
#[expected_failure(abort_code = 39, location = options_vault::vault)] // vault_positions_pending
...
#[test]
#[expected_failure(abort_code = 53, location = options_vault::vault)] // vault_proceeds_unswapped
...
#[test]
#[expected_failure(abort_code = 43, location = options_vault::vault)] // vault_strike_out_of_band
...
#[test]
#[expected_failure(abort_code = 44, location = options_vault::vault)] // vault_expiry_out_of_band
...
#[test]
#[expected_failure(abort_code = 47, location = options_vault::vault)] // vault_deposits_paused
...
#[test]
#[expected_failure(abort_code = 41, location = options_vault::vault)] // vault_round_not_finalized
...
#[test]
#[expected_failure(abort_code = 42, location = options_vault::vault)] // vault_receipt_round_mismatch
...
#[test]
#[expected_failure(abort_code = 7, location = auction::auction)] // settle_coupled
fun test_generic_settle_rejects_coupled_auction() {
    // The permissionless auction settle must not be able to strand a
    ...
    ts::next_tx(&mut scenario, keeper());
    let a = ts::take_shared_by_id<Auction<BTC, USDC>>(&scenario, rfq_id);
    auc::settle(a, &clock, scenario.ctx());
    clock.destroy_for_testing();
    ts::end(scenario);
}

#[test]
#[expected_failure(abort_code = 48, location = options_vault::vault)] // vault_wrong_origin
fun test_settle_rfq_rejects_foreign_auction() {
    ...
    let foreign_id = auc::create<BTC, USDC>(
...
#[test]
#[expected_failure(abort_code = 54, location = options_vault::vault)] // vault_config_invalid
...
fun test_swap_auction_fills_in_band() {
    ...
    ts::next_tx(&mut scenario, keeper());
    let a = ts::take_shared_by_id<Auction<USDC, BTC>>(&scenario, swap_id);
    assert!(auc::reserve_bid(&a) == 208, 0);
    ts::return_shared(a);
