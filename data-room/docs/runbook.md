...
`collector.toml`, systemd units (from `data-room/deploy/`).
...
1. Adapter in `data-room/crates/adapters/` (parse + `route` + golden
...
```bash
DATA_ROOM=s3://$BUCKET duckdb -init data-room/catalog/catalog.sql
