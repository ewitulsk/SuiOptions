...
//! `schema::BookL2Writer`; nothing holds a day. At 200 ms diff cadence
    ...
    for symbol in &symbols {
                ...
                lines_total +=
                    for_each_bronze_payload(store, key, &mut rejects, |line_no, ts, payload| {
                        let frame = match (exchange, stream.split_once('.').map(|s| s.0)) {
                            ("bluefin", Some("book")) => {
                                adapters::bluefin::parse_book(payload, ts, key, line_no)?
                            }
                            ("bluefin", Some("depth")) => {
                                adapters::bluefin::parse_depth_rest(payload, ts, key, line_no)?
                            }
                            ("deepbook", Some("book")) => {
                                adapters::deepbook::parse_book(payload, symbol, ts, key, line_no)?
                            }
                            _ => return Ok(()),
                        };
        ...
        if rows_total > 0 {
            ...
            let partition = match exchange {
                "bluefin" => adapters::bluefin::partition_symbol(symbol),
                _ => adapters::deepbook::partition_symbol(symbol),
            };
            put_bytes(
                store,
                &schema::silver_key("book_l2", exchange, &partition, date),
                bytes,
            )
            .await?;
            partitions += 1;
...
mod tests {
    ...
    async fn bluefin_day_is_deterministic_and_merges_diffs_with_rest_snapshots() {
        ...
        let key = root.join(schema::silver_key("book_l2", "bluefin", "SUI-PERP", date));
    ...
    async fn deepbook_snapshots_dedup_on_venue_timestamp() {
        ...
        let key = root.join(schema::silver_key("book_l2", "deepbook", "SUI-USDC", date));
        let rows = read_rows(&key);
    ...
    async fn unknown_exchange_is_an_error_and_empty_day_writes_nothing() {
        let tmp = tempfile::tempdir().unwrap();
        let store = store::open(&format!("file://{}", tmp.path().display())).unwrap();
