...
pub async fn normalize_day(store: &Store, date: &str) -> anyhow::Result<usize> {
    ...
    for symbol in &symbols {
        ...
        for day in [date, next.as_str()] {
            let prefix = schema::bronze_ws_prefix("bluefin", &format!("funding.{symbol}"), day);
            for key in keys.iter().filter(|k| k.starts_with(&prefix)) {
                lines_total +=
                    for_each_bronze_payload(store, key, &mut rejects, |line_no, ts, payload| {
                        for r in adapters::bluefin::parse_funding_history(
                            payload,
                            Some(ts),
                            key,
                            line_no,
        ...
        let seed_prefix = format!(
            "{}hour=23/",
            schema::bronze_ws_prefix("bluefin", &ticker_stream, &prev)
        );
        let day_prefix = schema::bronze_ws_prefix("bluefin", &ticker_stream, date);
            ...
            .filter(|k| k.starts_with(&seed_prefix) || k.starts_with(&day_prefix))
        {
            lines_total +=
                for_each_bronze_payload(store, key, &mut rejects, |line_no, ts, payload| {
                    let t = adapters::bluefin::parse_ticker(payload, key, line_no)?;
                        ...
                        if t.next_funding_ms > prev_next {
                            if day_of_ms(prev_next) == date {
                                derived.push(FundingRate {
                                    ...
                                    ts_recv: Some(ts),
                                    exchange: adapters::bluefin::EXCHANGE.into(),
                                    instrument_id: adapters::bluefin::instrument_id(&t.symbol),
                                    rate: t.last_rate,
                                    interval_hours: adapters::bluefin::FUNDING_INTERVAL_HOURS,
                                    kind: "derived".into(),
        ...
        if !derived.is_empty() {
            let bytes = schema::write_parquet(&schema::funding_rates_batch(derived)?)?;
            put_bytes(
                store,
                &schema::funding_silver_key("bluefin", &partition, date, "derived"),
                bytes,
            )
            .await?;
            written += 1;
...
mod tests {
    ...
    async fn rollovers_and_history_land_in_separate_parts_and_cross_check() {
        ...
        let settled = read_rows(&root.join(schema::funding_silver_key(
        ...
        let derived = read_rows(&root.join(schema::funding_silver_key(
        ...
        let before = std::fs::read(root.join(schema::funding_silver_key(
            "bluefin", "SUI-PERP", date, "derived",
        )))
        .unwrap();
        normalize_day(&store, date).await.unwrap();
        let after = std::fs::read(root.join(schema::funding_silver_key(
            "bluefin", "SUI-PERP", date, "derived",
        )))
        .unwrap();
