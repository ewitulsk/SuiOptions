    ...
    for coin in coins {
        ...
        for (i, r) in rows.iter().enumerate() {
            ...
            by_day.entry(day).or_default().push(FundingRate {
                ...
                exchange: adapters::hyperliquid::EXCHANGE.into(),
                instrument_id: adapters::hyperliquid::instrument_id(&r.coin),
                rate: r.funding_rate.parse()?,
                interval_hours: adapters::hyperliquid::FUNDING_INTERVAL_HOURS,
                kind: "settled".into(),
                mark_price: None,
                index_price: None,
        ...
        let symbol = adapters::hyperliquid::partition_symbol(coin);
        for (day, rows) in by_day {
            let bytes = schema::write_parquet(&schema::funding_rates_batch(rows)?)?;
            put_bytes(
                store,
                &schema::funding_silver_key("hyperliquid", &symbol, &day, "settled"),
                bytes,
            )
            .await?;
            partitions += 1;
