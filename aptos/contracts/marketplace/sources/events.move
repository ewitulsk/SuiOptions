...
/// Every event carries the fungible-asset `quote` (metadata object address)
/// the trade is priced in and the `fee` (our commission, in quote units) so
/// the indexer never needs a second lookup to attribute volume.
module marketplace::events {
    ...
    /// Collection object address for override lookup, or `@0x0` for TokenV1
    /// (which has no on-chain collection object).
    public fun token_collection_address(metadata: &TokenMetadata): address {
        if (option::is_some(&metadata.collection)) {
            object::object_address(option::borrow(&metadata.collection))
        } else {
            @0x0
        }
    }
