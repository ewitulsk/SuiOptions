// Sui data-plane clients, copied from frontend/src/lib/suiGrpc.ts (see
// docs/sui-json-rpc-migration.md): all chain reads, simulations, and
// transaction execution go through gRPC-web on the public fullnodes.
// dapp-kit's `SuiClientProvider` (a JSON-RPC client) stays mounted only for
// wallet plumbing — nothing routes RPC calls through it.
...
// gRPC-web rides the fullnodes' regular HTTPS port (CORS `*`).
